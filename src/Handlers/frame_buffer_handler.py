"""
Frame Buffer Handler - Thread-safe ring buffer that sits between input sources and consumers.
...
if __name__ == "__main__":
    ...
    config = Config()
    buf = FrameBufferHandler(config)

    # Simulate writing frames
    for i in range(901):
        fake_frame = np.zeros((480, 640, 3), dtype=np.uint8)
        buf.write_frame(fake_frame)
        time.sleep(0.03)

    print(f"✓ Buffer size: {buf.size}/{buf.capacity}")
    print(f"  Duration: {buf.duration_seconds:.2f}s")
    print(f"  Total written: {buf.total_frames_written}")

    latest = buf.get_latest()
    print(f"  Latest frame shape: {latest.shape}" if latest is not None else "  No frame")

    clip = buf.get_clip(0.2)
    print(f"  Clip (last 0.2s): {len(clip)} frames")
